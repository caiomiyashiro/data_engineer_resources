...
os.environ['AWS_SESSION_TOKEN']=config['CREDENTIALS']['AWS_SESSION_TOKEN']
...
def create_spark_session():
    ...
    AWS credentials are obtained throught env. variables. 
    """
    # hadoop package is used to read data from s3
    spark = SparkSession \
        .builder \
...
def process_song_data(spark: SparkSession, input_data: str, output_data: str) -> None:
    """
    Read song dataset, process it and store processed version in S3
    
    Args:
        spark: Spark context used to access spark commands
        input_data: S3 base directory
        output_data: S3 target directory
    """
...
#     df.createOrReplaceTempView("songs")
    ...
    songs_table.write.mode("overwrite").partitionBy("year", "artist_id").parquet(path=output_data + "song_data")
...
def process_log_data(spark: SparkSession, input_data: str, output_data: str) -> None:
    """
    Read log dataset, process it and store processed version in S3
